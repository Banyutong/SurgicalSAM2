...
def visualize_all_frames(video_segments, frame_names, video_dir, output_dir, gt_data,
                         prompt_frame_index=0, prompt_points=None, gt_type="pixel_mask",  class_to_color_mapper=None,
                         show_first_frame=True, show_points=True, point_positive_or_negative_labels=None, point_class_labels=None, ):
...
def visualize_frame(current_frame, prompt_frame_gt, ground_truth, prediction, output_path, prompt_frame=None, prompt_points=None,
    ...
    if show_first_frame:
        ...
        if gt_type == 'pixel_mask':
            ...
            axes[0].imshow(prompt_frame_gt, alpha=0.7)
        else:
            colors = get_color_map_255(len(prompt_frame_gt))
            for i, gt in enumerate(prompt_frame_gt):
                color = tuple(c / 255 for c in colors[i])  # Normalize color to 0-1 range for matplotlib
                ...
                if gt_type == 'bbox':
                    ...
                    contours, _ = cv2.findContours(gt.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                    for contour in contours:
                        axes[0].add_patch(plt.Polygon(contour.reshape(-1, 2), fill=True, alpha=0.4, color=color))


        if show_points:
            colors = get_color_map(len(prompt_points))
            if point_class_labels is None:
